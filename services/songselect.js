      ...
      jsdom.env(requestBody, ['http://code.jquery.com/jquery.min.js'], function (jsdomError, window) {
        if (jsdomError) {
          ...
          cache[id] = {
            ...
            authors: window.$.map(window.$('.authors a'), function (el) {
              return window.$(el).text();
            }),
            excerpt: window.$('.lyrics-prev').html().split('<br>').map(function (line) {
              return line.trim();
            }).join('\n'),
            year: parseInt(window.$('.copyright').html().split(' ')[1], 10),
            songType: 'chant'
          };
          if (cache[id].year) {
            if (cache[id].year > 2000) {
              cache[id].songType = 'new';
            } else if (cache[id].year > 1939) {
              cache[id].songType = 'old';
            }
          }
        ...
        if (jsdomError) {
          ...
          window.$('.song-listing tr > td:first-of-type').each(function (index, element) {
            cache[name].push({
              ...
              authors: window.$.map(window.$(element).find('.authors li'), function (el) {
                return window.$(el).text();
              }),
